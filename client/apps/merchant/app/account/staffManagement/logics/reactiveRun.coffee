Apps.Merchant.staffManagementReactive.push (scope) ->
  if Session.get("myProfile")
    staffs = Schema.staffs.find({parentMerchant: Session.get("myProfile").parentMerchant}).fetch()
    ...
    if Session.get("staffManagementSearchFilter")?.length > 0
      ...
      groupedStaffs = _.groupBy staffs, (staff) -> staff.name.split(' ').pop().substr(0, 1).toLowerCase()
    ...
    if Session.get("staffManagementSearchFilter")?.trim().length > 1
      if scope.managedStaffList.length > 0
        staffNameLists = _.pluck(scope.managedStaffList, 'name')
